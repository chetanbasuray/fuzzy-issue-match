...
function scorePair(pair) {
    return weightedFuzzyScore(pair.newIssue.title, pair.existingIssue.title, pair.newIssue.body, pair.existingIssue.body, { title: 0.6, body: 0.4 });
}
function metricValue(numerator, denominator) {
    return denominator === 0 ? 0 : numerator / denominator;
}
/** Run precision/recall/F1 measurements at each supplied threshold. */
export function runBenchmark(dataset, thresholds) {
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === "--dataset" && value) {
            ...
            console.log("Usage: npm run benchmark -- [--dataset path] [--thresholds 0.3,0.5] [--output path] [--check-baseline path] [--min-f1 0.5]");
            process.exit(0);
        }
        else {
...
function main() {
    ...
    if (options.minimumF1 !== undefined &&
    ...
    if (options.baselinePath) {
        const expected = JSON.parse(readFileSync(options.baselinePath, "utf8"));
